...
bool GameScene::init()
{
	...
	if (GameManager::getInstance()->getPlayer() == 2 || GameManager::getInstance()->getPlayer() == 3)
	{
		MyTank2 = Tank::create();
		MyTank2->setPlayer(2);
...
void GameScene::setPlayerTank()
{
	...
	if (x2 && (GameManager::getInstance()->getPlayer() == 2 || GameManager::getInstance()->getPlayer() == 3))
	{
		MyTank2 = Tank::create();
		MyTank2->setPlayer(2);
		MyTank2->setCamp(true);
...
{
	if (GameManager::getInstance()->getPlayer() == 3)
	{
		checkMsg();
	}
	if (nowAward != NULL)
	...
	if (GameManager::getInstance()->getClick3() == ESCBUTTON_UP||recv1 == 'e'||recv2 == 'e')
	{
	...
	if (GameManager::getInstance()->getPlayer() == 2 || GameManager::getInstance()->getPlayer() == 3)
	{
		...
		if (GameManager::getInstance()->getClick4() == NUMBERZEROBUTTON_ON)
		{
			MyTank2->fire();
		}
	}
...
void GameScene::checkMsg()
{
	send1 = "0";
	send2 = "0";
	if (GameManager::getInstance()->getClick1() == UPBUTTON_ON)
	{
		send1 = "u";
	}
	if (GameManager::getInstance()->getClick1() == LEFTBUTTON_ON)
	{
		send1 = "l";
	}
	if (GameManager::getInstance()->getClick1() == DOWNBUTTON_UP)
	{
		send1 = "d";
	}
	if (GameManager::getInstance()->getClick1() == RIGHTBUTTON_UP)
	{
		send1 = "r";
	}
	...
	if (GameManager::getInstance()->getClick2() == NUMBERZEROBUTTON_ON || GameManager::getInstance()->getClick2() == ENTERBUTTON_ON)
	{
		send2 = "f";
	}
	GameManager::getInstance()->getClient()->SendMsg(send1, 1);
	GameManager::getInstance()->getClient()->SendMsg(send2, 1);
	recv1 = GameManager::getInstance()->getClient()->Listening();
	recv2 = GameManager::getInstance()->getClient()->Listening();
}